...
router.get('/fetchallProfile',fetchuser,  async (req, res) => {
    try {
        ...
        // const Profiles = await Profile.find({ Profile_name: req.user.Profile });
        if (req.user.role !== "admin" && req.user.role !== "superadmin") {
            return res.status(401).json({ status: "error", message: "Not Allowed" });
        }
        const profiles = await Profile.find().select("-user");
        Status=200
        res.json(profiles);
    } catch (error) {
...
router.post('/addProfile', fetchuser, [
    ...
    try {
        ...
        const shortUuid= `ST${uuidv4()}`;
        const id = shortUuid.substr(0, 6);
        const username = `${first_name}${id}`;
        const newProfile = new Profile({ user: req.user.id, first_name,last_name,branch,username,city,state,zip,id });
